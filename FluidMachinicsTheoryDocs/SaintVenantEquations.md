...
方程成立的前提是基于恒定流假定，质量力只有重力，同时两个过水断面上为渐变流、断面间没有流量进出。
...
2. 流体的压缩性以 **体积压缩率$\kappa$** 或 **体积模数$K$** 表征，$K = \frac{1}{\kappa} = \frac{\rho \mathrm{d} p}{\mathrm{d} \rho}$；对于水，$K_{10℃} = 2.9 \times 10^9 (P_a)$。

3. 能量方程成立要求两个过水断面上水流为渐变流，但不要求断面之间水流也是渐变流。

4. 总水头线沿程的降低值与流程长度之比，称为总水头坡度，也称**水力坡度**： $J = \frac{H_{up} - H_{dn}}{L} = \frac{h_w}{L}$。

5. 明渠水流的控制体边界，一般既有底部和侧壁的固体边界，也有自由水面边界，横向边界一般取过水断面。
